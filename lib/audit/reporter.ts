...
// Keys match the tiers emitted by aggregateRun() in aggregate.ts
// ("Low" | "Moderate" | "High" | "Critical"), uppercased by riskBadge().
...
export function buildMarkdownReport(
  ...
  // Local date (en-CA → YYYY-MM-DD) so the report date matches the dashboard's
  // locally-formatted dates instead of drifting a day via UTC.
  ...
  lines.push(``);
  lines.push(
    `*Generated by Guardrail Auditor — suite ${versions.suiteVersion} · evaluator ${versions.evaluatorVersion} · executor ${versions.executionVersion}*`
  );
